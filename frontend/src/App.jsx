...
const gp = (p) => (parseFloat(p.sellPrice) || 0) - (parseFloat(p.cost) || 0);
const gm = (p) => { const sp = parseFloat(p.sellPrice) || 0; return sp > 0 ? (gp(p) / sp * 100) : 0; };
...
const B = { border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13 };
...
export default function App() {
  ...
  const Field = ({ field, label, type = 'text' }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <label style={{ color: MUTED, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em' }}>{label}</label>
      <input type={type} value={form[field] ?? ''} onChange={e => setForm(f => ({ ...f, [field]: type === 'number' ? parseInt(e.target.value) || 0 : e.target.value }))} style={inp} />
    </div>
  );
  ...
  // GP/GM live calc values
  const cSP = parseFloat(form.sellPrice) || 0;
  const cCO = parseFloat(form.cost) || 0;
  const cGP = cSP - cCO;
  const cGM = cSP > 0 ? (cGP / cSP * 100) : 0;
  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', color: TEXT, fontFamily: 'system-ui, sans-serif', fontSize: 13 }}>
      ...
      <div style={{ borderBottom: '1px solid ' + BORD, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => openModal('add')} style={{ ...B, background: ACCENT, color: '#fff', borderRadius: 6, padding: '7px 16px', fontWeight: 700 }}>+ New project</button>
      </div>
      ...
      <div style={{ overflowX: 'auto', padding: '0 20px 40px' }}>
        {filtered.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <tbody>
              {filtered.map(p => {
                const gpv = gp(p), gmv = gm(p), c = sc(p.status);
                  ...
                  <tr key={p.id} style={{ borderBottom: isExp ? 'none' : '1px solid #1e1e1e' }}>
                    <td style={{ padding: '9px 10px' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 0 }}>
                        {hasDl ? (
                          <button onClick={() => toggleExpand(p.id)} style={{ ...B, background: 'transparent', color: isExp ? '#888' : '#444', fontSize: 10, padding: '2px 4px 0 0', lineHeight: 1, flexShrink: 0 }}>
                            {isExp ? '▼' : '▶'}
                          </button>
                        ...
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {p.name}
                            {hasDl && <span style={{ fontSize: 10, color: dlColor, marginLeft: 6, fontWeight: 600 }}>{doneCnt}/{dl.length}</span>}
                          </div>
                    ...
                    <td style={{ padding: '9px 10px', whiteSpace: 'nowrap' }}>
                      <button onClick={() => openModal('edit', p.id)} style={{ ...B, background: '#2a2a2a', color: '#888', borderRadius: 4, padding: '4px 9px', marginRight: 3 }}>Edit</button>
                      <button onClick={() => setDelTarget(p.id)} style={{ ...B, background: 'transparent', color: '#ef444455', border: '1px solid #ef444433', borderRadius: 4, padding: '4px 9px' }}>Del</button>
                    </td>
                  </tr>,
            ...
            {delTarget ? (
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                  <button onClick={() => setDelTarget(null)} style={{ ...B, background: 'transparent', border: '1px solid ' + BORD2, color: DIM, borderRadius: 6, padding: '8px 20px' }}>Cancel</button>
                  <button onClick={handleDelete} style={{ ...B, background: '#ef4444', color: '#fff', borderRadius: 6, padding: '8px 20px', fontWeight: 700 }}>Delete</button>
                </div>
              </div>
            ) : (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  ...
                  <button onClick={() => setModal(null)} style={{ ...B, background: 'transparent', color: MUTED, fontSize: 22, lineHeight: 1 }}>x</button>
                </div>

                <Section label="Project info" />
                <Grid mb={14}>
                  <Field field="name" label="Project name" />
                  <Field field="customer" label="Customer / GC" />
                  <Sel field="status" label="Status" options={Object.keys(STATUSES)} />
                  <Sel field="pm" label="PM assignment" options={PMS} />
                </Grid>

                <Hr /><Section label="Financials" />
                <Grid mb={14}>
                  <Field field="sellPrice" label="Selling price ($)" type="number" />
                  <Field field="cost" label="Our cost ($)" type="number" />
                ...
                <Grid mb={14}>
                  <Field field="bidDueDate" label="Bid due date" type="date" />
                  <Field field="submittedDate" label="Date submitted" type="date" />
                  <Field field="awardDate" label="Award date" type="date" />
                  <Field field="projectStartDate" label="Project start date" type="date" />
                  <Field field="fabStartDate" label="Our fabrication start" type="date" />
                  <Field field="deliveryDate" label="Target final delivery" type="date" />
                  <Field field="changeOrders" label="Change orders (#)" type="number" />
                </Grid>
                ...
                {formDeliveries.map((d, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '130px 1fr auto auto', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <input type="text" value={d.desc || ''} onChange={e => setFormDeliveries(prev => prev.map((x, j) => j === i ? { ...x, desc: e.target.value } : x))} placeholder="Description (e.g. Phase 1 — columns and beams)" style={{ ...inp }} />
                    ...
                    <button onClick={() => setFormDeliveries(prev => prev.filter((_, j) => j !== i))} style={{ ...B, background: 'transparent', border: '1px solid #ef444433', color: '#ef444466', borderRadius: 4, padding: '3px 8px', fontSize: 11 }}>Remove</button>
                  </div>
                ))}
                <button onClick={() => setFormDeliveries(prev => [...prev, { id: Date.now().toString(), date: '', desc: '', done: false }])} style={{ ...B, background: 'transparent', border: '1px solid ' + BORD2, color: MUTED, borderRadius: 4, padding: '6px 14px', fontSize: 11, marginTop: 4 }}>+ Add delivery phase</button>

                <Hr /><Section label="Galvanizing" />
                <Grid mb={14}>
                  <Field field="galvSendDate" label="Sent to galvanizer" type="date" />
                  <Field field="galvReturnDate" label="Est. return from galv" type="date" />
                </Grid>

                <Hr /><Section label="Paint" />
                <Grid mb={14}>
                  <Field field="paintSendDate" label="Paint start date" type="date" />
                  <Field field="paintCompleteDate" label="Est. paint complete" type="date" />
                </Grid>

                <Hr /><Section label="Materials and drawings" />
                <Grid mb={14}>
                  <Sel field="drawingStatus" label="Drawing status" options={DRW} />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <label style={{ color: MUTED, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.08em' }}>Material ordered</label>
                    <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
                      {[true, false].map(v => (
                        <button key={String(v)} onClick={() => setForm(f => ({ ...f, materialOrdered: v }))} style={{ ...B, flex: 1, padding: '7px 0', border: '1px solid ' + (form.materialOrdered === v ? (v ? '#22c55e' : '#ef4444') : BORD2), background: form.materialOrdered === v ? (v ? '#22c55e22' : '#ef444422') : 'transparent', color: form.materialOrdered === v ? (v ? '#22c55e' : '#ef4444') : MUTED, borderRadius: 4, fontWeight: 600, fontSize: 12 }}>{v ? 'Yes' : 'No'}</button>
                      ))}
                    </div>
                  </div>
                </Grid>
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                  <button onClick={() => setModal(null)} style={{ ...B, background: 'transparent', border: '1px solid ' + BORD2, color: DIM, borderRadius: 6, padding: '8px 18px' }}>Cancel</button>
                  <button onClick={handleSave} disabled={saving} style={{ ...B, background: saving ? BORD2 : ACCENT, color: '#fff', borderRadius: 6, padding: '8px 22px', fontWeight: 700, opacity: saving ? 0.6 : 1 }}>{saving ? 'Saving...' : modal === 'add' ? 'Add project' : 'Save changes'}</button>
                </div>
              </>
            )}
          </div>

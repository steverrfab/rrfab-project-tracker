...
const GROUPS = { All: ps => ps, 'Needs setup': ps => ps.filter(p => p.needsSetup), Backlog: ps => ps.filter(p => p.status === 'Awarded'), Active: ps => ps.filter(p => ACTIVE_STAGES.includes(p.status)), Completed: ps => ps.filter(p => p.status === 'Completed'), 'On Hold': ps => ps.filter(p => p.status === 'On Hold') };
